...
public class SPacketPlayerPositionRotationMesh extends PacketMesh.Server<PlayerPositionLookS2CPacket> implements SPacketPlayerPositionRotation {
    ...
    public Set<Flag> getFlags() {
        ...
        for(PlayerPositionLookS2CPacket.Flag flag: getMeshValue().getFlags()) {
            flagSet.add(Flag.VALUES[flag.ordinal()]);
        }
        return flagSet;
    }

    public static final PlayerPositionLookS2CPacket.Flag[] FLAGS = PlayerPositionLookS2CPacket.Flag.values();
    @Override
    ...
    public Boolean getShouldDismount() {
        return getMeshValue().shouldDismount();
    }

    @Override
    public void setShouldDismount(boolean value) {
        ((PlayerPositionLookS2CPacketAccessor) getMeshValue()).setShouldDismount(value);
    }
}
